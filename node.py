...
class OpenRouterNode:
    ...
    RETURN_NAMES = ("Output", "Stats", "Credits")
    ...
    def generate_response(self, api_key, system_prompt, user_message_box, model,
        ...
        Returns three outputs:
          ...
          (2) Stats: a string with tokens per second, prompt tokens, completion tokens
          (3) Credits: a string with the user's credit information
        """
        if not api_key:
             return ("Error: API Key not provided.", "Stats N/A", "Credits N/A")
        ...
        if pdf_data is not None:
            ...
            if isinstance(pdf_data, dict) and "bytes" in pdf_data and isinstance(pdf_data["bytes"], bytes):
                ...
                try:
                    ...
                    return (f"Error encoding PDF: {e}", "Stats N/A", "Credits N/A")
            else:
        ...
        # Append the complete user message (always using the array format if image or PDF is present)
        ...
        try:
            ...
            text_output = result["choices"][0]["message"].get("content", "") # Safely get content
            ...
            return (error_message, "Stats N/A due to error", "Credits N/A due to error")
        except Exception as e: # Catch other potential errors (e.g., JSON parsing, value errors)
             return (f"Node Error: {str(e)}", "Stats N/A due to error", "Credits N/A due to error")
